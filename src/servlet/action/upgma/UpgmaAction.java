...
public class UpgmaAction extends Action implements ProcessMultipartAction {
...
System.err.println("name="+name+" filename="+filename);
	...
	fileWasNull = true;
      ...
      if (name.equals("data_file")) {
        try {
          ff1 = File.createTempFile("marray_upgma_in_","");
        } catch (IOException e) {
	  e.printStackTrace();
        }
System.err.println("name="+ff1.getName()+" path="+ff1.getAbsolutePath());
    ...
    try {
      ff2 = File.createTempFile("marray_upgma_image_",".png",new File(Constants.TEMP_DIRECTORY_VISIBLE));
    } catch (IOException e) {
      e.printStackTrace();
    }
    ff2.deleteOnExit();
    ...
    try {
      ...
      result = Computation.upgma(args,report);
    } catch (Exception e) {
      e.printStackTrace();
      report.appendLine(""+e);
      result = false;
    ...
    UpgmaBean upgmaBean = new UpgmaBean();
    upgmaBean.setMessage(message);
    upgmaBean.setImage(image);

    session.setAttribute("bean", upgmaBean);
    forward("/jsp/upgma_result.jsp",request,response);
  }
}
